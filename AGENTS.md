...
- Registry gained rows for legacy model ids (fable-5, opus-4-8, opus-4-7, sonnet-4-6) and known-wrong Sonnet 5 / Opus 5.5 tier-default prices were left stale, silently under-billing 25-50% → Before adding registry price rows, cross-check the latest-ids-only policy first: a consumer pinning an old id is a bug to report against the consumer, not a row to add to the registry. And never ship a known-wrong price for a live tier default as a deferred follow-up, fix it in the same card even if it touches a pinned test file, or block merge and say so explicitly
